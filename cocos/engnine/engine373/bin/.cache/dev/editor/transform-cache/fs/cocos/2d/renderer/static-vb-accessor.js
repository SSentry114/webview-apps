    ...
    execute: function () {
      ...
      _export("StaticVBAccessor", StaticVBAccessor = class StaticVBAccessor extends BufferAccessor {
        ...
        constructor(device, attributes, vCount, iCount) {
          ...
          this._id = StaticVBAccessor.generateID(); // Initialize first mesh buffer
        ...
        reset() {
          for (let i = 0; i < this._buffers.length; ++i) {
            const buffer = this._buffers[i]; // Reset index buffer
        ...
        uploadBuffers() {
          for (let i = 0; i < this._buffers.length; ++i) {
            ...
            const buffer = this._buffers[i]; // Recognize active buffers
        ...
        recycleChunk(chunk) {
          ...
          let i = 0;
          let prevEntry = null;
          let nextEntry = freeList[i]; // Loop entries
          ...
          if (prevEntry) {
            const distance = offset - (prevEntry.offset + prevEntry.length); // Ensure no overlap with previous chunk
          ...
          if (!recycled && nextEntry) {
            const distance = nextEntry.offset - (offset + bytes); // Ensure no overlap with next chunk
