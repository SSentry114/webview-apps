System.register("q-bundled:///fs/cocos/rendering/custom/web-program-library.js", ["../../gfx/index.js", "../../render-scene/core/program-utils.js", "../../render-scene/index.js", "./layout-graph.js", "./types.js", "./web-types.js", "./layout-graph-utils.js", "../../core/platform/debug.js", "../define.js"], function (_export, _context) {
  ...
  function calculateFlattenedBinding(descriptorSets, fixedInstanceDescriptorSetLayout, shaderInfo) {
    ...
    const uniformBlockCapacities = new Array(4);
    {
      ...
      const batchOffset = instanceOffset + instanceCapacity; // save uniform block offsets by set index
    ...
    }
    {
      ...
      const phaseCapacity = ((_descriptorSets$Updat6 = descriptorSets[UpdateFrequency.PER_PHASE]) === null || _descriptorSets$Updat6 === void 0 ? void 0 : _descriptorSets$Updat6.samplerTextureCapacity) || 0; // const batchCapacity = descriptorSets[UpdateFrequency.PER_BATCH]?.capacity || 0; // dynamic size
      ...
      const phaseOffset = passOffset + passCapacity;
      const instanceOffset = phaseOffset + phaseCapacity;
      const batchOffset = instanceOffset + instanceCapacity; // save sampler texture offsets by set index
      ...
      samplerTextureOffsets[_setIndex[UpdateFrequency.PER_INSTANCE]] = instanceOffset; // update flattened sampler texture binding
  ...
  _export("WebProgramLibrary", void 0);
  ...
  return {
    ...
    execute: function () {
      _setIndex = [2, 1, 3, 0];
      WebProgramProxy = class WebProgramProxy {
        constructor(shader) {
          this.shader = void 0;
          this.shader = shader;
        }
      ...
      _export("WebProgramLibrary", WebProgramLibrary = class WebProgramLibrary {
        ...
        addEffect(effect) {
          ...
          for (const tech of effect.techniques) {
            for (const pass of tech.passes) {
              ...
              const phaseLayout = lg.getLayout(phaseID); // programs
              ...
              const phasePrograms = group.programInfos; // build program

              const programInfo = makeProgramInfo(effect.name, srcShaderInfo); // collect program descriptors
              ...
              const [shaderInfo, blockSizes] = makeShaderInfo(lg, passLayout, phaseLayout, srcShaderInfo, programData, this.fixedLocal); // overwrite programInfo

              overwriteProgramBlockInfo(shaderInfo, programInfo); // handle map

              const handleMap = genHandles(shaderInfo); // attributes
        ...
        getProgramVariant(device, phaseID, name, defines, key = null) {
          ...
          const shader = device.createShader(shaderInfo); // create program host and register
