System.register("q-bundled:///fs/cocos/tiledmap/assembler/simple.js", ["../../../../virtual/internal%253Aconstants.js", "../../core/math/index.js", "../tiled-types.js", "../../game/index.js", "../../2d/renderer/static-vb-accessor.js", "../../2d/renderer/vertex-format.js", "../../2d/renderer/render-data.js", "../../2d/renderer/render-draw-info.js"], function (_export, _context) {
  ...
  */
  ...
  function traverseGrids(leftDown, rightTop, rowMoveDir, colMoveDir, comp) {
    ...
    _fillCount = 0;
    const vertexBuf = _tempBuffers; // traverse row

    for (; (rows - row) * rowMoveDir >= 0; row += rowMoveDir) {
      rowData = vertices[row];
      colNodesCount = comp.getNodesCountByRow(row);
      isCheckColRange = rowData && colNodesCount === 0; // limit min col and max col
      ...
      for (; (cols - col) * colMoveDir >= 0; col += colMoveDir) {
        ...
        if (!tiledNode) {
          if (diamondTile) {
            ...
            vec3_temps[0].y = top; // lc

            vec3_temps[1].x = left;
            vec3_temps[1].y = centerY; // rc

            vec3_temps[2].x = right;
            vec3_temps[2].y = centerY; // cb
            ...
            // lt
            vec3_temps[0].x = left;
            vec3_temps[0].y = top; // lb

            vec3_temps[1].x = left;
            vec3_temps[1].y = bottom; // rt

            vec3_temps[2].x = right;
            vec3_temps[2].y = top; // rb
        ...
        _fillCount++; // vertices count exceed 66635, buffer must be switched
      ...
      _export("simple", simple = {
        ensureAccessor() {
          if (!_accessor) {
            ...
            _accessor = new StaticVBAccessor(device, vfmtPosUvColor, this.vCount); //batcher.registerBufferAccessor(Number.parseInt('TILED-MAP', 36), _accessor);
        ...
        fillBuffers(layer, renderer) {
          ...
          const dataArray = layer.tiledDataArray; // 当前渲染的数据
