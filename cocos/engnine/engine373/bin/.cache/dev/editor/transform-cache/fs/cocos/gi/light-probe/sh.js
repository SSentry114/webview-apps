    ...
    execute: function () {
      ...
      _export("SH", SH = class SH {
        ...
        static shaderEvaluate(normal, coefficients) {
          ...
          const normal4 = new Vec4(normal.x, normal.y, normal.z, 1.0); // calculate linear and const terms
      ...
      SH.LMAX = 2;
      SH.basisFunctions = [v => 0.282095, // 0.5 * Math.sqrt(1.0 / Math.PI)
      v => 0.488603 * v.y, // 0.5 * Math.sqrt(3.0 / Math.PI) * v.y
      v => 0.488603 * v.z, // 0.5 * Math.sqrt(3.0 / Math.PI) * v.z
      v => 0.488603 * v.x, // 0.5 * Math.sqrt(3.0 / Math.PI) * v.x
      v => 1.09255 * v.y * v.x, // 0.5 * Math.sqrt(15.0 / Math.PI) * v.y * v.x
      v => 1.09255 * v.y * v.z, // 0.5 * Math.sqrt(15.0 / Math.PI) * v.y * v.z
      v => 0.946175 * (v.z * v.z - 1.0 / 3.0), // 0.75 * Math.sqrt(5.0 / Math.PI) * (v.z * v.z - 1.0 / 3.0)
      v => 1.09255 * v.z * v.x, // 0.5 * Math.sqrt(15.0 / Math.PI) * v.z * v.x
