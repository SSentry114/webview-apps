      ...
      _export("CSMLayers", CSMLayers = class CSMLayers {
        ...
        _splitFrustumLevels(dirLight) {
          ...
          for (let i = 1; i < level; i++) {
            // i ÷ numbers of level
            const si = i / level; // eslint-disable-next-line no-restricted-properties

            const preNear = lambda * (nd * Math.pow(ratio, si)) + (1 - lambda) * (nd + (fd - nd) * si); // Slightly increase the overlap to avoid fracture
