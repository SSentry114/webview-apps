    ...
    execute: function () {
      ...
      _export("Batcher2D", Batcher2D = class Batcher2D {
        ...
        get currBufferAccessor() {
          if (this._staticVBBuffer) return this._staticVBBuffer; // create if not set
        ...
        switchBufferAccessor(attributes = vfmtPosUvColor) {
          const strideBytes = attributes === vfmtPosUvColor ? 36
          /* 9x4 */
          : getAttributeStride(attributes); // If current accessor not compatible with the requested attributes
        ...
        updateBuffer(attributes, bid) {
          const accessor = this.switchBufferAccessor(attributes); // If accessor changed, then current bid will be reset to -1, this check will pass too
        ...
        endStaticBatch() {
          this.finishMergeBatches();
          this.currStaticRoot = null; // Clear linear buffer to switch to the correct internal accessor
        ...
        autoMergeBatches(renderComp) {
          ...
          const accessor = this._staticVBBuffer; // Previous batch using mesh buffer
          ...
          if (rd && rd._isMeshBuffer) {
            ...
            buf.setDirty(); // Request ia

            ia = buf.requireFreeIA(this.device);
            ia.firstIndex = this._indexStart;
            ia.indexCount = indexCount; // Update index tracker and bid

            this._indexStart = buf.indexOffset;
          }

          this._currBID = -1; // Request ia failed
        ...
        walk(node, level = 0) {
          ...
          if (!approx(opacity, 0, EPSILON)) {
            ...
            if (uiProps.colorDirty) {
              ...
              this._opacityDirty--; // Reset color dirty
