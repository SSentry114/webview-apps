System.register("q-bundled:///fs/cocos/render-scene/core/program-lib.js", ["../../../../virtual/internal%253Aconstants.js", "../../rendering/define.js", "../../gfx/index.js", "./program-utils.js", "../../core/index.js"], function (_export, _context) {
  ...
  function findDefineIndependent(source, tmpl, attrMap, locSet) {
    ...
    let code = source; // layout(location = 3) in mediump vec3 v_normal;
    // 3
    // v_normal

    while (locExistingRes) {
      const attrName = locExistingRes[2];
      const attrInfo = tmpl.attributes.find(ele => ele.name === attrName); // no define required.
  ...
  function replaceVertexMutableLocation(source, tmpl, macroInfo, inOrOut, attrMap, locSet = new Set()) {
    ...
    while (locHolder) {
      ...
      if (!attrMap.has(attrName)) {
        ...
        let active = true;
        let location = 0; // only vertexshader input is checked

        if (inOrOut === 'in') {
          const targetStr = source.slice(0, locHolder.index); // attrInfo?.defines store defines need to be satisfied
          ...
          active = !!(attrInfo !== null && attrInfo !== void 0 && attrInfo.defines.every(defStrIn => {
            ...
            if (res) {
              ...
              if (lastIfRes) {
                ...
                const evalORElements = evalStr.split('||'); // simple grammar, no parenthesses support yet.
                ...
                const evalRes = evalORElements.some(eleOrTestStr => {
                  ...
                  return evalANDElements.every(eleAndTestStr => {
                    ...
                    } else {// no compare just define or not
                      // expect to be true
                    }
        ...
        if (active) {
          ...
          locSet.add(location); // const attrInfo = tmpl.attributes.find((ele) => ele.name === attrName);
  ...
  function replaceFragmentLocation(source, inOrOut, attrMap) {
    let code = source;
    const locHolderRegStr = `layout\\(location = ([^\\)]+)\\)\\s+${inOrOut}.*?\\s(\\w+)[;,\\)]`;
    const locHolderReg = new RegExp(locHolderRegStr, 'g'); // layout(location = 3) in mediump vec3 v_normal;
    // 3
    // v_normal
  ...
  function processShaderInfo(tmpl, macroInfo, shaderInfo) {
    ...
    shaderInfo.stages[1].source = flattenShaderLocation(shaderInfo.stages[1].source, tmpl, macroInfo, 'frag', attrMap); // don't forget to change location 'shaderInfo.attributes' which comes from serialization
