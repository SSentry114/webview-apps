  ...
  return {
    ...
    execute: function () {
      ...
      _export("MeshBuffer", MeshBuffer = class MeshBuffer {
        ...
        set vData(val) {
          this._vData = val; //还得看是否需要共享.buffer
        ...
        destroy() {
          ...
          this._iaInfo = null;
          this.vData = null;
          this.iData = null; // Destroy InputAssemblers
        ...
        uploadBuffers() {
          ...
          if (iOS14 && submitCount / this._iaPool.length < IA_POOL_USED_SCALE) {
            ...
            const length = this._iaPool.length; // Destroy InputAssemblers
        ...
        createNewIA(device) {
          let ia;
          let vertexBuffers;
          let indexBuffer; // HACK: After sharing buffer between drawcalls, the performance degradation a lots on iOS 14 or iPad OS 14 device
          ...
          if (sys.__isWebIOS14OrIPadOS14Env || !this._iaPool[0]) {
            ...
            vertexBuffers = [vertexBuffer]; // Reuse purpose for new IAs
