    ...
    execute: function () {
      ...
      _export("StaticVBAccessor", StaticVBAccessor = /*#__PURE__*/function (_BufferAccessor) {
        ...
        function StaticVBAccessor(device, attributes, vCount, iCount) {
          ...
          _this._id = StaticVBAccessor.generateID(); // Initialize first mesh buffer
        ...
        _proto2.reset = function reset() {
          for (var i = 0; i < this._buffers.length; ++i) {
            var buffer = this._buffers[i]; // Reset index buffer
        ...
        _proto2.uploadBuffers = function uploadBuffers() {
          for (var i = 0; i < this._buffers.length; ++i) {
            ...
            var buffer = this._buffers[i]; // Recognize active buffers
        ...
        _proto2.allocateChunk = function allocateChunk(vertexCount, indexCount) {
          ...
          var bid = 0;
          var eid = -1;
          var entry = null; // Loop buffers

          for (var i = 0; i < this._buffers.length; ++i) {
            buf = this._buffers[i];
            freeList = this._freeLists[i]; // Loop entries
        ...
        _proto2.recycleChunk = function recycleChunk(chunk) {
          ...
          var i = 0;
          var prevEntry = null;
          var nextEntry = freeList[i]; // Loop entries
          ...
          if (prevEntry) {
            var distance = offset - (prevEntry.offset + prevEntry.length); // Ensure no overlap with previous chunk

            assertIsTrue(distance >= 0); // Can be merged

            if (distance === 0) {
              prevEntry.length += bytes;
              offset = prevEntry.offset;
              bytes = prevEntry.length; // Can also merge with next entry

              if (nextEntry && nextEntry.offset - (offset + bytes) === 0) {
                prevEntry.length += nextEntry.length; // Free next entry
          ...
          if (!recycled && nextEntry) {
            var _distance = nextEntry.offset - (offset + bytes); // Ensure no overlap with next chunk


            assertIsTrue(_distance >= 0); // Can be merged
        ...
        _proto2._allocateBuffer = function _allocateBuffer() {
          ...
          this._freeLists.push(freeList); //sync to native
