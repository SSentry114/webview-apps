...
#import "View.h"
#include <string>
#include "platform/apple/DeviceUID.h"
...
//    
//    // Log event
//        [FIRAnalytics logEventWithName:@"app_launch"
//                            parameters:@{
//                                @"platform": @"ios"
//                            }];
...
+ (NSString *)getInfo:(NSNumber *)methodID
                    andParam0:(NSString *)param0
                    andParam1:(NSString *)param1
{
    NSString *ret = @"";
    AppBridgeMethod mid = (AppBridgeMethod)[methodID integerValue];

    switch (mid) {
        case AppBridgeMethodGetVendorID: {
            ret = [DeviceUID uid];
            break;
        }
            
        case AppBridgeMethodRotate: {
            NSNumber* orientation = @([param0 intValue]);
            [ViewController SetDeviceOrientation:orientation];
            break;
        }
            
        case AppBridgeMethodHideLoadingView: {
            [AppDelegate hideLoadingView];
            break;
        }

        case AppBridgeMethodGetTimeZoneName: {
            NSTimeZone *tz = [NSTimeZone localTimeZone];
            ret = tz.name ?: @"";
            break;
        }
        case AppBridgeMethodGetBundleID: {
            ret = [NSBundle mainBundle].bundleIdentifier ?: @"";
            break;
        }
        case AppBridgeMethodOpenSettings: {
        ...
        default: {
            // Chặn toàn bộ ID không khai báo để tránh “hidden behavior”
            // Có thể trả chuỗi lỗi, hoặc rỗng
            ret = @"";
            break;
        }
    }
